...
class DoctorReviews extends StatelessWidget {
  ...
  DoctorReviews({Key? key, required this.allDoctorsData}) : super(key: key);
    ...
    return CustomCard(
      column: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Icon(Icons.star,color: Colors.orangeAccent,size: 20,) , Text(allDoctorsData.avgRatingStars.toString()),SizedBox(width: 4,),Text("(${allDoctorsData.data!.reviews?.length})")
            ],),
          ...
          Expanded(
              child: ListView.builder(
                ...
                itemCount: allDoctorsData.data!.reviews?.length,
                itemBuilder: (context, index) {
                  return ReviewCard(
                      name: 'API Needed for user\'s name & image',
                      image: reviews[index].image,
                      rating: allDoctorsData.data!.reviews?[index].stars,
                      reviewText:allDoctorsData.data!.reviews?[index].
                      reviewText);
                },
              ))
        ],
